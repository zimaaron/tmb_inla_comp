...
for(dl in c('binom', 'normal')){
  for(inla.int in c('EB_G', "EB_S", "EB_L", 'CCD_G', "CCD_S", "CCD_L")){
    for(with.tmb in c("only", "not", "both")){ # plot TMB only, INLA only, both [on the same plots]
      ...
      ## rename for plotting
      setnames(long.cov.sum, 'n.clust',  'Number Obs')
      ...
      if(dl=='binom'){
        fit_coverage_CI_summary <- fit_coverage_CI_summary +
          facet_wrap(. ~ `Number Obs`, labeller = label_both)
      }
      if(dl == 'normal'){
        fit_coverage_CI_summary <- fit_coverage_CI_summary +
          facet_grid(`Obs Var` ~ `Number Obs`, labeller = label_both)
      }
...
(summary.metrics[,`Int. Bias` := fe_int_mean - alpha])
(summary.metrics[,`Obs Var Bias` := gauss_var_mean - norm.var])
(summary.metrics[,`Matern Range Bias` := matern_range_mean - sp.range])
(summary.metrics[,`Matern SD Bias` := matern_sigma_mean - sqrt(sp.var)])
(summary.metrics[,`Clust Var Bias` := clust_var_mean - clust.var])
(summary.metrics[,`Access Coef Bias` := fe_access2_mean - eval(parse(text=betas))[1]]) 
# NOTE! assumes access2 is first and mapincidence is second! count be written better....
(summary.metrics[,`Mal. Inc. Coef Bias` := fe_mapincidence_mean - eval(parse(text=betas))[2]])

non.pix.eff.b.colnames <- c('Int. Bias', 
                            'Obs Var Bias',
                            'Matern Range Bias',
                            'Matern SD Bias',
                            'Clust Var Bias',
...
for(dl in c('binom', 'normal')){
  for(inla.int in c('EB_G', "EB_S", "EB_L", 'CCD_G', "CCD_S", "CCD_L")){
    for(with.tmb in c("only", "not", "both")){ # plot TMB only, INLA only, both [on the same plots]
      ...
      ## rename for plotting
      setnames(fe.mean.long.sum, 'n.clust',  'Number Obs')
      ...
      fe_bias_summary <- ggplot(fe.mean.long.sum,
        ...
        xlab('Number of Observed Spatial Locations') + 
        scale_x_continuous(breaks = log(sort(fe.mean.long.sum$`Number Obs`)), ## add log x-axis labels
                           labels = paste0('ln(', sort(fe.mean.long.sum$`Number Obs`), ')')) + 
...
loop.params <- data.table(expand.grid(dl=c('binom', 'normal'),
                                      ...
                                      inla.strat = c('EB_G', # "EB_S",  "EB_L", 
                                                     #'CCD_G', "CCD_S", 
                                                     "CCD_L"),
                                      num.mesh = summary.metrics[,max(st_mesh_nodes)]))
...
for(ii in 1:loop.params[,.N]){
  ...
  ## rename for plotting
  setnames(cov.gp.sum, 'n.clust',   'Number Obs')
  setnames(cov.gp.sum, 'clust.var', 'Clust Var')
...
loop.params <- data.table(expand.grid(dl=c('binom', 'normal'),
                                      ...
                                      inla.strat = c('EB_G',  "EB_S",  "EB_L", 
                                                     'CCD_G', "CCD_S", "CCD_L"),
                                      num.mesh = summary.metrics[,unique(st_mesh_nodes)]))
...
long.cov.sum <-  summarySE(long.cov, measurevar="time_s",
                           groupvars=c("operation","fit_type", 'n.clust', 'st_mesh_nodes'))
...
fit_coverage_CI_summary <- ggplot(long.cov.sum,
  ...
  geom_line() +
  geom_point() +
  facet_wrap(st_mesh_nodes ~ operation) + ggtitle(sprintf('Comparison of fit time (sec) in: %s', loopvars$data.lik[1]))
...
ggsave(sprintf('%s/%s_fit_time_num_res.png', compar.dir, loopvars$data.lik[1]),
       plot = fit_coverage_CI_summary,
       device = 'png', units = 'in',
       width = 12, height = 12)
