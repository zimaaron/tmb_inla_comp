...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_SCALAR(logsigmamean); // mean of prior for logsigma
  ...
  PARAMETER(logsigma);         // log of sd for iid random effect

  PARAMETER_VECTOR(S);    // spatial random effects
  PARAMETER_VECTOR(nu);    // iid random effects
  ...
  nll -= dnorm(logsigma, logsigmamean, Type(1.0), true);
  ...
  // IID random effects
  for (int i = 0; i < num; i++) {
    nll -= dnorm(nu(i), Type(0.0), exp(logsigma), true);
  }
  ...
  for (int i = 0; i < num; i++) {
    nll -= dbinom_robust( y(i), n(i), alpha + projS(i) + nu(i), true );
  }
